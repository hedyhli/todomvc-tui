...
let init _model = Command.Set_timer (timer_ref, 1.0)

(* Update function with pattern matching for events *)
let update event model =
  match event with
  | Event.Timer _ref -> ({model with alted = true}, Command.Enter_alt_screen)
...
let view model =
  (* if model.quitting then "asdfasdfa\n" *)
  if not model.alted then ""
  else
    let open Spices in
